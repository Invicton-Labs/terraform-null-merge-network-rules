locals {
  ...
  range_0_with_sort_keys = length(local.range_keys) <= 0 ? {} : {
    ...
    group_key => merge(group, {
      rules = [
        ...
        merge(rule, {
          sort_key = contains(keys(rule.ranges), local.range_keys[group_key][0]) ? "${format("%0${local.digit_count}d", rule.from_inclusive == null ? local.neg_inf : rule.from_inclusive)}-${format("%0${local.digit_count}d", rule.to_inclusive == null ? local.pos_inf : rule.to_inclusive)}-${rule_idx}" : "nokey-${rule_idx}"
        })
      ]
    })
  }
    ...
    group_key => merge(group, {
      rules = [
        ...
        merge(rule, {
          ...
          metadata = flatten(
            [
              for squashed_idx in range(rule_idx, rule_idx + rule.contiguous_forward_count) :
  ...
  #   // Squash contiguous rules for the first range key
  #   range_0_next_contiguous = {
  #     for group_key, group in local.reverse_pass_encapsulate :
  #     key => merge(group, {
  #       rules = [
  ...
  #           // - For every range other than the one being considered, the next rule's value is exactly equal
  #           // - For every discrete key, the next rule's value is exactly equal
  #           contiguous__type_with_next = (
  #             // Check if it's the last rule.
  #             // If this index is equal to the last index value, bail out with "false" because it's
  #             // the last rule and therefore cannot be contiguous with the next rule.
  #             rule_idx == length(goup.rules) - 1 ? false : (
  #               // Check if the range we're considering is actually contiguous
  #               // It's contiguous with the next rule if:
  #               // - Both rules have the key, AND
  #               //    - This rule's "to" value is greater than or equal to one less than the next rule's "from" value (forward contiguity), OR
  #               //    - This rule's "from" value is less than, equal, or one greater than the next rule's "to" value (reverse contiguity)
  #               // This rule doesn't contain the range key we're checking? That means it can't be contiguous.
  #               !contains(keys(rule.ranges), range_key) ? false : (
  #                 // The next rule doesn't contain the range key we're checking? That means it can't be contiguous.
  #                 !contains(keys(group.rules[rule_idx + 1].ranges), range_key) ? false : !(
  #                   // First, check forward contiguity (next rule has higher values)
  #                   // If this rule's value we're comparing has a null "to", that means we can't compare on the upper (forward) side.
  #                   // This case should be filtered out in the encapsulation merging.
  #                   rule.ranges[local.range_keys[group_key][0]].to_inclusive == null ? false : (
  #                     // If the next rule's value we're comparing has a null "from", that means we can't compare on the upper (forward) side.
  ...
  #                       // We have two real values to compare, so compare them.
  #                       // They are forward contiguous if this rule's "to" value is at least adjacent (one less) to the next rule's "from" value
  #                       rule.ranges[local.range_keys[group_key][0]].to_inclusive >= group.rules[rule_idx + 1].ranges[local.range_keys[group_key][0]].from_inclusive - 1
  #                     )
  #                   )
  #                   ? true : (
  #                     // If it's not forward contiguous, it might be reverse contiguous, so check that.
  #                     // If this rule's value we're comparing has a null "from", that means we can't compare on the lower (reverse) side.
  #                     // This case should be filtered out in the encapsulation merging.
  #                     rule.ranges[local.range_keys[group_key][0]].from_inclusive == null ? false : (
  #                       // If the next rule's value we're comparing has a null "to", that means we can't compare on the lower (reverse) side.
  #                       // This case should be filtered out in the encapsulation merging.
  #                       group.rules[rule_idx + 1].ranges[local.range_keys[group_key][0]].to_inclusive == null ? false : (
  #                         // We have two real values to compare, so compare them.
  #                         // They are forward contiguous if this rule's "to" value is at least adjacent (one less) to the next rule's "from" value
  #                         rule.ranges[local.range_keys[group_key][0]].from_inclusive <= group.rules[rule_idx + 1].ranges[local.range_keys[group_key][0]].from_inclusive + 1
  #                       )
  #                     )
  #                   )
  #                   ) ? false : (
  ...
  #   // Above, we found the rules that were contiguous
  #   range_0_squashed = {
  #     for group_key, group in local.reverse_pass_encapsulate :
  #     key => merge(group, {
  #       rules = [
  ...
  #         })
  #         if(
  #           // Only include this rule if the previous one wasn't contiguous,
  #           // because if it was then the previous one will be expanded
  #           true
  #         )
  #       ]
  #     })
